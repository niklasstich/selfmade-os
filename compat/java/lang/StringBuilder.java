...
public class StringBuilder {
	...
	public StringBuilder(int capacity) {
		if(capacity<0) MAGIC.inline(0xCC);
		length = 0;
	...
	private void appendInternal(char[] val) {
		while(length+val.length>=capacity) expandBuffer();
		for (char c : val) {
			appendInternal(c);
		}
	}
	...
	private void appendInternal(char c) {
		while(length+1>=capacity) expandBuffer();
		buf[length] = c;
		length++;
	}
	...
	public void ensureSize(int size) {
		if (size>capacity) expandBufferInternal(size);
	}
